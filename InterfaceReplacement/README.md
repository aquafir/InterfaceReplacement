...
## 🔧 Developing
  ...
  * The plugin code is located in the `InventoryUI` project. You can ignore the `InventoryUI.Loader` project, it contains the hot-reloader and doesn't need to be edited unless you are doing something out of the ordinary.
    * `InventoryUI/PluginCore.cs` is the main Plugin class. It contains the plugin startup / shutdown methods.
    * `InventoryUI/ExampleUI.cs` includes some demo UI code.
    * `InventoryUI/scripts/installer.nsi` is the NSIS script used to generate the installer.
  * Build the new plugin solution in Visual Studio with `Ctrl+Shift+B`, or by going to `Build -> Build Solution`.
  * Add both `InventoryUI.dll` and `InventoryUI.Loader.dll` to decal by opening decal from the tray and selecting `Add -> Browse -> <YourProjectPath>/bin/Release/net481/`. (Select each dll file individually.)
  * Disable `InventoryUI` in decal by unchecking it under the `Plugins` list.
    * During development you should have `InventoryUI` (under `Plugins`) disabled, and `InventoryUI.Loader` enabled (under `Network Filters`) in decal. This allows for hot-reloading of the plugin without logging out / restarting the client.
...
## 📦 Releasing
  ...
  * In decal, enable your Plugin under the `Plugins` section, and disable `InventoryUI.Loader` under Network Filters. This allows you to test the plugin with hot-reloading disabled.
